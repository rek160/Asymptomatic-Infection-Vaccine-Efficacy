...
# Function to take results of an epidemic and analyse the data to get vaccine efficacy estimates
  ...
  num_communities<-5
  rate_within<-0.009627992 #0.007702394, 0.009627992  0.011553591
  rate_between<-0.0002673774 #0.0002139019 0.0002673774 0.0003208528
  ...
  directory<-'~/RCode_RK/5comm1.25/'
  filename<-paste0(directory,"5comm_constant_Results_Analysis",cluster_coverage,"_",trial_length,"_",j,"_",betaAS,"_",vpS,"_",rate_within,"_",sample_percent,".csv")
  ...
  if (nrow(results_analysis)>1){
    ...
    if ((numevents_vacc>0)&&(numevents_cont>0)) {
      survmodel<-try(coxph(Surv(DayInfected,eventstatus)~TrialStatus+strata(Community),results_analysis),silent=T)
      ...
      survmodel<-try(coxph(Surv(DayInfected,eventstatus)~TrialStatus+strata(Community),results_analysis1),silent=T)
      usesurvmod <- !inherits(survmodel, 'try-error')
    ...
    if ((numevents_vacc_S>0)&&(numevents_cont_S>0)) {
      survmodel<-try(coxph(Surv(DayInfected,eventstatus)~TrialStatus+strata(Community),results_analysis_VES),silent=T)
      usesurvmod <- !inherits(survmodel, 'try-error')
      if (!is.na(survmodel$coefficients[1])){
      ...
      survmodel<-try(coxph(Surv(DayInfected,eventstatus)~TrialStatus+strata(Community),results_analysis_VES1),silent=T)
      usesurvmod <- !inherits(survmodel, 'try-error')
    ...
    VEC_corrected_comm <- c()
    se_corrected <- c()
    for (i in 1:num_communities){
      results_analysis_i <- results_analysis[results_analysis$Community==i,]
      a <- nrow(results_analysis_i[(results_analysis_i$eventstatus==1) & (results_analysis_i$TrialStatus==1),])
      e <- nrow(results_analysis_i[(results_analysis_i$TrialStatus==1),])
      c <- nrow(results_analysis_i[(results_analysis_i$eventstatus==1) & (results_analysis_i$TrialStatus==0),])
      f <- nrow(results_analysis_i[(results_analysis_i$TrialStatus==0),])
      b <- e - a
      d <- f - c
      AR_vacc<-a/e
      AR_cont<-c/f
      VEC_corrected_1comm<-1-log(1-AR_vacc)/log(1-AR_cont)
      VEC_corrected_comm <- c(VEC_corrected_comm,VEC_corrected_1comm)
      var_RRC<- (a/(e*b) + ((1-VEC_corrected)^2)*c/(e*d))/
        (log(e/(e-c)))^2
      se_corrected <- c(se_corrected,sqrt(var_RRC))
    }
    
    VEC_corrected_meta <- c()
    meta_analysis <- metagen(VEC_corrected_comm, se_corrected,method.tau = "DL")
    VEC_corrected_meta[1] <- meta_analysis$TE.fixed
    VEC_corrected_meta[2] <- meta_analysis$lower.fixed
    VEC_corrected_meta[3] <- meta_analysis$upper.fixed
    VEC_corrected_meta_var <- meta_analysis$seTE.fixed^2
    VEC_corrected_meta_pval <- meta_analysis$pval.fixed
    ...
    for (i in 1:num_communities){
      ...
      AR_vacc<-a/e
      AR_cont<-c/f
      RR_1comm<-1-AR_vacc/AR_cont
      RR_comm <- c(RR_comm,RR_1comm)
      var_RR <- (b/(a*(a+b)))+(d/(c*(c+d)))
      RR_se <- c(RR_se,sqrt(var_RR))
    }
    
    RR_meta <- c()
    meta_analysis <- metagen(RR_comm,RR_se,method.tau = "DL")
    RR_meta[1] <- meta_analysis$TE.fixed
    RR_meta[2] <- meta_analysis$lower.fixed
    RR_meta[3] <- meta_analysis$upper.fixed
    RR_meta_var <- meta_analysis$seTE.fixed^2
    RR_meta_pval <- meta_analysis$pval.fixed
    ...
    ## Run interval censored model in each community and meta analyze
    ## Meta analysis of the 5 communities -- interval censored
    betas <- c()
    se.betas <- c()
    for (k in 1:num_communities){
      results_analysis_int_comm <- results_analysis_int[results_analysis_int$Community==k,]
      if (numevents_vacc>0 && numevents_cont>0){
        survmodel<- try(ic_sp(cbind(DayEnrolled, DayInfected) ~ TrialStatus, model = 'ph',
                              bs_samples = 50, data = results_analysis_int_comm))
        ...
        # What time to assign? I have chosen to give both events the median time
        # among control events.
        results_analysis_int_comm2<-results_analysis_int_comm
        newevent_v_rownum <- min(which((results_analysis_int_comm2$eventstatus==0)&(results_analysis_int_comm2$TrialStatus==1)))
        newevent_c_rownum <- min(which((results_analysis_int_comm2$eventstatus==0)&(results_analysis_int_comm2$TrialStatus==0)))
        
        eventtime <- median(results_analysis_int_comm2$DayInfected[results_analysis_int_comm2$eventstatus==1])
        
        results_analysis_int_comm2$DayInfected[newevent_v_rownum] <- eventtime
        results_analysis_int_comm2$eventstatus[newevent_v_rownum] <- 1
        
        results_analysis_int_comm2$DayInfected[newevent_c_rownum] <- eventtime
        results_analysis_int_comm2$eventstatus[newevent_c_rownum] <- 1
        
        survmodel<- try(ic_sp(cbind(DayEnrolled, DayInfected) ~ TrialStatus, model = 'ph',
                              bs_samples = 50, data = results_analysis_int_comm2))      
        usesurvmod <- !inherits(survmodel, 'try-error')
      ...
      betas <- c(betas, survmodel$coefficients)
      se.betas <- c(se.betas, sqrt(VEC_int_var))
    }
    vaccEffEst_total_int <- c()
    meta_analysis <- metagen(betas, se.betas,method.tau = "DL")
    vaccEffEst_total_int[1] <- 1-exp(meta_analysis$TE.fixed)
    vaccEffEst_total_int[2] <- 1-exp(meta_analysis$upper.fixed)
    vaccEffEst_total_int[3] <- 1-exp(meta_analysis$lower.fixed)
    VEC_int_var <- meta_analysis$seTE.fixed^2
    VEC_pval_int <- meta_analysis$pval.fixed
    ...
    ## Run interval censored model in each community and meta analyze
    ## Meta analysis of the 5 communities -- interval censored 3x
    betas <- c()
    se.betas <- c()
    for (k in 1:num_communities){
      results_analysis_int_1_comm <- results_analysis_int_1[results_analysis_int_1$Community==k,]
      if ((numevents_vacc>0)&&(numevents_cont>0)) {
        survmodel<- try(ic_sp(cbind(DayEnrolled, DayInfected) ~ TrialStatus, model = 'ph',
                              bs_samples = 50, data = results_analysis_int_1_comm))
        if (!is.na(survmodel$coefficients[1])){
          # If no error was thrown, use the results of the model
        ...
        # What time to assign? I have chosen to give both events the median time
        # among control events.
        results_analysis_int_1_comm2<-results_analysis_int_1_comm
        newevent_v_rownum <- min(which((results_analysis_int_1_comm2$eventstatus==0)&(results_analysis_int_1_comm2$TrialStatus==1)))
        newevent_c_rownum <- min(which((results_analysis_int_1_comm2$eventstatus==0)&(results_analysis_int_1_comm2$TrialStatus==0)))
        
        eventtime <- median(results_analysis_int_1_comm2$DayInfected[results_analysis_int_1_comm2$eventstatus==1])
        
        results_analysis_int_1_comm2$DayInfected[newevent_v_rownum] <- eventtime
        results_analysis_int_1_comm2$eventstatus[newevent_v_rownum] <- 1
        
        results_analysis_int_1_comm2$DayInfected[newevent_c_rownum] <- eventtime
        results_analysis_int_1_comm2$eventstatus[newevent_c_rownum] <- 1
      ...
      betas <- c(betas, survmodel$coefficients)
      se.betas <- c(se.betas, sqrt(VEC_int_1_var))
    }
    vaccEffEst_total_int_1 <- c()
    meta_analysis <- metagen(betas, se.betas,method.tau = "DL")
    vaccEffEst_total_int_1[1] <- 1-exp(meta_analysis$TE.fixed)
    vaccEffEst_total_int_1[2] <- 1-exp(meta_analysis$upper.fixed)
    vaccEffEst_total_int_1[3] <- 1-exp(meta_analysis$lower.fixed)
    VEC_int_1_var <- meta_analysis$seTE.fixed^2
    VEC_pval_int_1 <- meta_analysis$pval.fixed
    ...
    for (k in 1:10){
      ...
      se.betas <- c()
      for (k in 1:num_communities){
        results_analysis_impute_int_comm <- results_analysis_impute_int[results_analysis_int$Community==k,]
        if (numevents_vacc>0 && numevents_cont>0){
          survmodel<- try(ic_sp(cbind(DayEnrolled, DayInfected) ~ TrialStatus, model = 'ph',
                                bs_samples = 50, data = results_analysis_impute_int_comm))
          usesurvmod <- !inherits(survmodel, 'try-error')
          if (usesurvmod==TRUE){
            # If no error was thrown, use the results of the model
          ...
          # What time to assign? I have chosen to give both events the median time
          # among control events.
          results_analysis_impute_int_comm2<-results_analysis_impute_int_comm
          newevent_v_rownum <- min(which((results_analysis_impute_int_comm2$eventstatus==0)&(results_analysis_impute_int_comm2$TrialStatus==1)))
          newevent_c_rownum <- min(which((results_analysis_impute_int_comm2$eventstatus==0)&(results_analysis_impute_int_comm2$TrialStatus==0)))
          
          eventtime <- median(results_analysis_impute_int_comm2$DayInfected[results_analysis_impute_int_comm2$eventstatus==1])
          
          results_analysis_impute_int_comm2$DayInfected[newevent_v_rownum] <- eventtime
          results_analysis_impute_int_comm2$eventstatus[newevent_v_rownum] <- 1
          
          results_analysis_impute_int_comm2$DayInfected[newevent_c_rownum] <- eventtime
          results_analysis_impute_int_comm2$eventstatus[newevent_c_rownum] <- 1
          
          survmodel<- try(ic_sp(cbind(DayEnrolled, DayInfected) ~ TrialStatus, model = 'ph',
                                bs_samples = 50, data = results_analysis_impute_int_comm2))
        ...
        betas <- c(betas, survmodel$coefficients)
        se.betas <- c(se.betas, sqrt(VEC_impute_int_var))
      }
      vaccEffEst_total_impute_int <- c()
      meta_analysis <- metagen(betas, se.betas,method.tau = "DL")
      vaccEffEst_total_impute_int[1] <- 1-exp(meta_analysis$TE.fixed)
      vaccEffEst_total_impute_int[2] <- 1-exp(meta_analysis$upper.fixed)
      vaccEffEst_total_impute_int[3] <- 1-exp(meta_analysis$lower.fixed)
      VEC_impute_int_var <- meta_analysis$seTE.fixed^2
      VEC_pval_impute_int <- meta_analysis$pval.fixed      
  ...
  names <- c("num_vacc","numevents_vacc","numevents_vacc_S","numevents_vacc_AS",
             ...
             "VEC_corrected_meta","VEC_corrected_lower", "VEC_corrected_upper","VEC_corrected_meta_var","VEC_corrected_meta_pval",
             "RR_meta","RR_meta_low","RR_meta_high","RR_meta_var","RR_meta_pval","VEP_impute")
  ...
  results <- c(num_vacc,numevents_vacc,numevents_vacc_S,numevents_vacc_AS,
               ...
               vaccEffEst_total_impute_int[1], vaccEffEst_total_impute_int_lower,vaccEffEst_total_impute_int_upper, 
               VEC_var, VES_var, VEC_int_var,VEC_int_1_var, VEC_impute_int_var, VEC_pval_impute_int,
               VEC_corrected_meta[1],VEC_corrected_meta[2], VEC_corrected_meta[3],VEC_corrected_meta_var,VEC_corrected_meta_pval,
               RR_meta[1],RR_meta[2],RR_meta[3],RR_meta_var,RR_meta_pval,VEP_impute)
  
  
  trialresults<- rbind(names,results)
  write.csv(trialresults, paste0("meta_5comm_constant_trial_results_",cluster_coverage,"_",trial_length,"_",j,"_",betaAS,"_",vpS,"_",rate_within,"_",sample_percent,".csv"))
